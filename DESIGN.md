...
colors:
  wing-bg: "light-dark(#1c476e, #0f2539)"
  wing-fg: "light-dark(#ffffff, #ffffff)"
  wing-fg2: "light-dark(#d0dbe0, #bcc2c8)"
  wing-hi: "light-dark(#f28c28, #f28c28)"
  wing-on-hi: "light-dark(#091825, #091825)"
  wing-chip: "light-dark(#375d7f, #2c3f51)"
  wing-ring: "light-dark(#ff8ac2, #ff8ac2)"
  rail-bg: "light-dark(#122e47, #0b1a28)"
  rail-fg2: "light-dark(#bdc4cb, #bbbfc3)"
  rail-chip: "light-dark(#2e475d, #283542)"
...
- Motion: an overshoot for arrivals only, expo ease-out for everything else, a 360ms re-tint, and a story turn between clubs (the new club's colour poured over the screen, dripping). On the landing, a crowd of real bodies pops up in a pile that is different on every visit, can be knocked flying with the cursor and stays where it lands, and six of them fly down into the next chapter as you scroll. All of it is cut under reduced motion.
...
- **Club Deep** (`club-deep`, `wing-bg`): the kit's darker colour at HSL lightness 0.27 by day and 0.14 by night, settled until white holds 6.5:1 (9:1 by night) on it hovered. (A 12:1 night bar pushed the wing down to about 0.08, as dark as a night card, and a card opened over the wing vanished into it.) It is the wing; the rail is it mixed 30% toward black (`rail-bg`). By day it is the highlight on the stage and on cards (`stage-hi`, `card-hi`): the primary button, the send button, your reaction. By night the stage becomes a saturated version of it (`stage-bg`, lightness 0.21).
...
**Code Font:** JetBrains Mono (with ui-monospace, monospace): only for what is meant to be typed exactly, a command or a snippet in a message, so code never falls to whichever monospace the reader's system has.
...
- **Headline** (800, 2.75rem, 1, -0.03em): the start of a channel or conversation, the inbox title, the sign-up member card's name.
- **Nook Name** (800, 2rem, 0.95, -0.03em): the nook's name at the top of the wing, up to two lines, balanced.
- **Title** (800, 1.875rem, 1, -0.02 to -0.03em): dialog, thread and member-sheet heads (Edit profile included), a person card's name, empty-state lines.
...
### Named Rules
**The Two Volumes Rule.** Funnel Display 800 names things: nooks, channels, people in an opening, page, dialog and popover heads, day dividers, the wordmark, and the initials in faces and discs (drawn as the letter's outline, see the Steady Letter Rule). Everything read or pressed (messages, labels, controls, counts) is Funnel Sans.
...
### Shadow Vocabulary
- **Card** (`box-shadow: 0 0 0 1px light-dark(transparent, rgb(255 255 255 / 0.09)), 0 1px 2px light-dark(rgb(10 12 20 / 0.08), rgb(0 0 0 / 0.4)), 0 10px 30px -10px light-dark(rgb(10 12 20 / 0.28), rgb(0 0 0 / 0.65))`): cards resting on the colour: the composer, a message that mentions you, file attachments and images, the message action bar, the thread card beside the room.
- **Float** (`box-shadow: 0 0 0 1px light-dark(transparent, rgb(255 255 255 / 0.11)), 0 2px 6px light-dark(rgb(10 12 20 / 0.1), rgb(0 0 0 / 0.45)), 0 24px 56px -14px light-dark(rgb(10 12 20 / 0.36), rgb(0 0 0 / 0.75))`): things over the room: popovers, menus, tooltips, dialogs, drawers, the command palette, the landing's live demo.
...
**The Night Lip Rule.** By night a card is as dark as the wing and the rail it may open over, and a shadow on near-black barely shows, so every lifted card also carries a 1px hairline of white at 9% (resting) or 11% (floating) as the first layer of its shadow: the lip of something lifted catching the light. By day it is transparent. It is part of the shadow, not a border, and nothing docked ever has it.
...
- **Link:** bare `fg` type, underlined, set flush with the text around it (no side padding): "Make a different link".
- **Press / Focus / Disabled / Pending:** `:active` squashes to 90% scale and back over 180ms; focus is a 2.5px outline in the surface's `fg`, offset 2px; disabled drops to 50% opacity and loses the press; pending keeps its colour (it is busy, not unavailable) with its pending label and the progress cursor. Nothing is outlined to look clickable, and everything pressable shows the hand (The Hand Rule).

### Chips (reactions, counts, pills)
- **Reactions:** 32px pills in the surface's own chip colour under the message (so they read on the room and on a card alike: a card-coloured chip on a card vanished), carrying up to a few faces of who reacted, drawn in a card's face colours, and a tabular count. Yours flips to the room's highlight, extrabold, and re-colours the faces in it; `aria-pressed` carries the state.
...
- **Border:** none (by night the hairline is part of the shadow: The Night Lip Rule).
...
- **Error:** a 2px inset ring in alert, and the message below as a card pill with the warning glyph (The Alert Lives On A Card Rule); it takes the place of the field's helper line rather than repeating it. Messages are written for people ("At least 2 characters"), never a validator's own words.
- **Switch:** a 52×32 chip track holding a 24px knob centred in it, 4px from the end it rests at, sliding 20px; on, the track takes the highlight and the knob `on-hi`.
...
### Navigation
- **Rail:** 44px nook discs, 12px apart, each named by the card tooltip beside it; the current one sits in a 2.5px ring in `fg` with 6px between them; hover grows discs to 105%, press shrinks to 95%. Pointing at a nook loads it ahead (its channels, the channel it opens on and that channel's latest messages), so a switch lands on a room already drawn. The "new nook" disc is an uncoloured chip with a plus that fills with the highlight on hover. The inbox and your own shape sit at the foot.
- **Wing:** the nook's name (Nook Name), then one quiet line with the member count and a small Invite chip (32px, pop-coloured glyph) at its end, then the 44px search pill with its shortcut, then 44px channel rows in quiet `fg-2`, medium. Name first, search second, invite as an aside: the search pill is the only full-width control above the channels. Unread lifts a row to `fg` bold with a pop count; the current row is a solid pill of the highlight with `on-hi` text, a swatch of where you are, and that pill slides from row to row the moment you click (see Motion). DM rows show the person's shape with a presence mark.
- **Mobile:** a menu button beside the room title opens the rail and wing as one drawer.

### Presence
Shape first, colour second, each on a halo of the surface colour: **online** a filled disc in the surface's `ring` colour (pop on the wing and rail, ink by day and bright by night on the stage, highlight on cards); **away** a half-filled disc; **do not disturb** a bar; **offline** an open ring, the last three in `fg-2`. The mark sits on the person's own edge towards the bottom-right, a little outside it (`REACH_45` in `lib/faces`, measured from each path), so it touches a sparkle's notch or an arch's corner rather than floating in the box's empty corner. In the who's-here strip, people who are here also wear a 2.4px ring in their own outline, scaled 130% around the shape. Set over a banner (the person card, the sign-up member card), a person is cut out of it by a 6px band of the card's colour in their own outline, never by a disc their corners poke out of.
...
### Picture Viewer
A picture in a message opens over the room at full size on black at 85%, with its name, where you are ("2 of 3"), the original and close across the top of the screen. A message's pictures are one set, in the order they were attached: 48px round arrows at the screen's sides (hidden on phones, where a swipe does it), the arrow keys wherever focus is, and a row of dots to jump to any one; each picture comes in from the side the move went (320ms, expo), and the ones either side are fetched ahead. A click anywhere but the picture and its controls puts it away, as Escape does.
...
Changing nooks pours the new club's colour over the screen (`story-turn.ts`, the plan in `paint.ts`, the paint in `liquid.ts`, View Transitions). The browser snapshots the room being left and the new club is drawn; then a wave of the new club's paint (its room's highlight, so it stands out against the room it uncovers in either scheme: the deep colour by day, the bright one by night) comes down from the top-left corner and runs off the bottom-right one, and the new room fills in behind it. The paint is liquid: a band drawn every frame as one signed distance field by a WebGL shader, its front a blobby edge with swells that travel along it, and drips that grow out of it as it goes, each a stem that tapers to a neck above a round bead that swells and shrinks as it hangs; the drips near the middle (and some others, by chance) let a drop go that falls free, stretching, and the paint pulls back into a bead where it came from. Everything joins everything else with a smooth union, so the band, the drips and the drops run together like paint rather than overlapping like shapes. It is lit: a wet lip along every edge and a highlight where the paint catches the light (on a software renderer, a cheaper shading from the distance alone). The new room is uncovered by a clip-path the browser animates on the compositor, and the paint is drawn in a worker on an `OffscreenCanvas` on its own clock, set to the same start, so neither is held up by the room being drawn; the paint overlaps the room's edge by 14px and runs 18ms behind the clip so the join never shows. 820ms on a sine in-out: moving from the first frame, quickest through the middle. Pointing at a nook loads it ahead, the new room is drawn before the paint moves, and the paint waits (at most 180ms) for the room's first settling work to pass. The re-tint is off while it runs (`html[data-turn] .tint`), so the room arrives in its own colours, and the canvas is removed once the room has covered it. Without a worker that can draw WebGL, the same wave is laid as three shaped layers (paint, gloss, lip) over the clip instead. The rail, the Ctrl+K palette and the landing's club discs all pour; the landing's automatic advance just re-tints. Without View Transitions, or under reduced motion, the change is a plain cut.
...
### The Crowd (landing)
The first screen ends in a pile of people along its foot: the demo clubs' members and made-up others in all twenty-one shapes, in the club's face colours, re-tinting with it. They are real bodies (`crowd-physics.ts`, Box2D by way of planck.js): each person's outline is the convex hull of their shape, pushed out until it encloses the drawn curve, with mass that grows with their size, a good grip on one another and little bounce, in a room with a slippery floor, two walls, the top of the hero as a ceiling and, from `xl`, the live demo as a solid block they pile against and only the small ones fit under. Every visit heaps them differently: a pile is laid out from a fresh seed (`crowd-layout.ts`, with the six named people in a shuffled order along their stretch), settled out of sight (first with nobody turning, then free), and only then shown. The server renders a pile of its own from a fixed seed, so it stands there with no script. From `xl` the pile is a mound under the call to action with the six named people standing in front, and a trail of small people along the floor beneath the demo; a strip from `md`; a small one on phones. Only people at least 72% the size of the named six carry an initial, so it reads as a crowd, not an alphabet. The hero is clipped at its foot; the pile stands on that edge. From `xl` the crowd stands in front of the page's content, so people can be tapped and thrown across it; only a drawn shape takes the pointer, wherever the physics has put it.
...
- **Pour ease** (a sine in-out, `paint.ts`): the story turn only: moving from the first frame, quickest through the middle, settling at the end, like a pour.
...
- **In the app:** the current-channel pill slides to the row you picked the moment you click, before the channel has loaded (420ms, expo, a transform animated on the compositor, so drawing the channel you are going to never holds it up), and re-tints on its fill alone; a thread card slides in from the right (420ms, expo, a transform on the compositor) while the room gives way beside it on the same clock and curve, its conversation and composer narrowing as the card comes in, and closing, the room widens back as the card slides away, so the card's edge and the room's always meet; your own reaction pops its chip and a face joining a chip on screen slaps down beside the others (nothing pops for chips already there when the message appeared); the send arrow leaves through the top of its disc and a fresh one rises in (440ms); someone arriving in the who's-here strip grows their ring out from them (460ms, pop); picking a shape morphs the profile preview to it (650ms, `nook-morph`, a gentle in-out so the in-between shapes are seen, with a small 92%→100% pop): every shape is measured once as its radius at 120 angles round its centre, so the morph blends two lists of numbers each frame, with nothing to match up and nothing to twist.
- **On the landing:** the crowd pops up where it came to rest, quickly and in no order (each person from 20% scale and 18px above, 340ms, pop, all within about 0.7s), and is a pile you can play with: the cursor's whole path between two pointer events is tested against everyone, so a slow pass makes whoever it brushes flinch (a squash to 107%/90% from the feet and back, 380ms, on their own element, never the physics) and shuffles anyone with nobody resting on them a step aside at walking pace, without turning them, while a fast swipe kicks them into the air along it, spinning the way they were hit, and from there the engine takes over: they fly, knock into whoever they land on, and stay where they land (nobody is walked back to their old spot). Someone left on the floor with nobody leaning on them picks themselves up for a moment after they stop moving, so their initial reads again; a lean inside the pile stays. A tap throws someone up to turn over, passing through the people on their shoulders for 300ms so the jump is theirs. The crowd jumps when the page changes clubs (nearest the disc you pressed first). The loop sleeps with the pile: once nobody is moving, nothing runs. The club's name in the headline lands letter by letter (SplitText, 24ms apart). As the who's-here chapter comes up, its six people are lifted out of the pile from wherever the physics has left them (held still there, the others left standing round them) and fly down into their seats, scrubbed to scroll, and are put back where they were when you scroll back up, righting themselves on the way (however far they tumbled in the pile, the nearer way round: one turn eased in and out across the whole flight, with a sway of up to 16° the same way that fades for anyone with far to turn), and passing behind the chapter's words rather than across them: the pile and the row are each clipped to their own section, so a person changes colour exactly as they cross the edge between rooms, and on landing whoever is here draws their ring on and the names come up under them. Each later chapter plays its piece once as it comes on screen, the way the app would: a message lands and its reactions pop on, thread replies arrive one after another, the file drops in and the link unfolds, the search types itself and its hits light up. No heading or paragraph moves.
- **Reduced motion:** every transition and animation above is cut, the story turn is a plain cut, popups lose their scale, a thread is simply in place, and the settle flash holds as a static 10% wash. The crowd stands still where it rests, a swipe or a tap moves nobody, nobody flies, the headline swaps without letters landing, every chapter piece is simply there, the pill and the preview land at once, and the ring appears without growing.

### Named Rules
**The Steady Letter Rule.** A drawing that grows on hover (or while held, or while its popup is open) grows inside its own SVG: the box that takes the hover carries `data-grows`, and the group inside the drawing that scales carries `data-grow`, so the shape and its letter scale together as one drawing, never as a texture. Nothing that grows is put on a layer of its own (`will-change`): a layer is cut off at its bounds, blurs while it scales and snaps sharp when the hover ends. Initials in faces and discs are the letter's own outline (`<Initial>`, Funnel Display at 800 from `lib/initial-glyphs.ts`, written by `scripts/initial-glyphs.py`), not text: text puts its baseline on a whole pixel and jumps a pixel against a growing shape; an outline scales with it exactly (a letter outside Latin-1 falls back to text). `scripts/verify-steady-letters.mjs` measures it through the compositor.

**The Compositor Carries Travel Rule.** Something that travels while the app is busy drawing (the channel pill, the thread card, the nook pour) moves by a transform or a clip the browser animates for it, started on the click, never frame by frame from the main thread, so it keeps moving while a channel or a thread is being drawn; the pour's paint is drawn in a worker for the same reason.

**The Hand Rule.** Everything pressable shows the pointing hand: buttons, links, radios, switches, tabs, menu items, options, labels that wrap a control, and the landing's people. Tailwind's reset leaves buttons on the arrow, so the base layer restores it; a menu item is never set back to the arrow.
